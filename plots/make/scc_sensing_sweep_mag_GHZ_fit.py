...
def plot_sweep_mag_vs_g(
    ...
    for g in listof_g:
        ...
        # calculate Sx op with QuSpin using same symmetries as model
        Sxterms = [["x", [[1, i] for i in range(Ns)]]]
        basis = quspin.basis.spin_basis_general(Ns, S="1/2", **symmetries)
        checks = {"check_herm": False, "check_symm": False}
        Sxop = quspin.operators.hamiltonian(Sxterms, [], basis=basis, **checks)

        mags.append(Sxop.matrix_ele(final_wf, final_wf).real / Ns)  # per site
        gs.append(g)

    fig, ax = plt.subplots(figsize=(9, 5))
    ax.set_xlim(-1.05 * max(listof_g), 1.05 * max(listof_g))
    ax.plot(gs, mags, "k-", linewidth=2)
    ax.set_xlabel(r"$g$")
    ax.set_ylabel(r"$\langle S^x \rangle$")
    plt.savefig(
        "plots/images/sweep_X_mag_GHZ_fit_N{0}_ord{1}_tau{2}.pdf".format(
